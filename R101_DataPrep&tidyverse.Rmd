...
pacman::p_load(tidyverse) #note: need to load individual packages separately
```
...
Note that we only want ton include *adult* patients, let's do a quick cleaning up:
...
It is always highly recommended to calculate the proportions of missing data. There are multiple ways to do it in R, SQL and SAS. 

```{r missing_prop_sim, include=F}
#since all patients in our dataset has at least 1 BMI, for demnstration purpose, I will randomly remove few BMI out
asd_down_bmi_median$bmi_med[sample(1:20,3)]<-NA
```


```{r missing_prop}
##====================================================
# "describe" is a function from package "stats"
##====================================================
sum(is.na(asd_down_bmi_median$bmi_med))/nrow(asd_down_bmi_median)
```
...
* Model based imputation: 
    ...
    * Multiple-Value Imputation (sensitivity analysis)
...
* Multiple imputation approaches generally perform better than other methods.
    * Choice of the best imputation method
        ...
        * Sensitivity analysis approach:
            ...
            * 3. Reconstruct/Impute the missing data using the various methods.
            * 4. Compute the performance metrics (e.g. R_square, AUC, SSE) between the reconstructed and the original data, for each method and each proportion of missing data.
            * 5. Repeat steps 1–4 a number of times (10 times for example) and compute the average performance of each method.
            * 6. Plot the average performance vs. proportions of missing
            * 7. Choose the method that performs best at the level of missing data in your dataset.


**************************************************************************************************

#### Noise vs. Outlier (Chapter 14)
...
```{r bmi_density}
ggplot(asd_down_bmi_median,aes(x=bmi_med))+
  geom_density(aes(fill=DIAGNOSIS),alpha=0.5)
```


```{r bmi_transform, fig.width=10, fig.height=6}
asd_down_bmi_trans<-asd_down_bmi_median %>% mutate(trans_bmi=bmi_med, trans_type="0.original") %>%
  bind_rows(asd_down_bmi_median %>% mutate(trans_bmi=log(bmi_med), trans_type="1.log")) %>%
  bind_rows(asd_down_bmi_median %>% mutate(trans_bmi=bmi_med^(1/2), trans_type="2.sqrt")) %>%
  bind_rows(asd_down_bmi_median %>% mutate(trans_bmi=bmi_med^(1/3), trans_type="3.cubert"))
...
```{r outlier_tukey}
ggplot(asd_down_bmi_trans %>% filter(trans_type %in%  c("0.original","1.log")),
       aes(y=trans_bmi,x=DIAGNOSIS))+
  geom_boxplot()+
  facet_wrap(~trans_type,scales="free",nrow=1)
```
...
```{r}
bmi_med_m<-mean(asd_down_bmi_median$bmi_med,na.rm=T)
bmi_med_sd<-sd(asd_down_bmi_median$bmi_med,na.rm=T)

asd_down_bmi_z<-asd_down_bmi_median %>% 
  mutate(bmi_z=(bmi_med-bmi_med_m)/bmi_med_sd)
...
* Cook's Distance: in linear regression, cook's distance is commonly used to estimate the influence of a data point on the coefficients. 
    ...
    * Some commonly-used cut-off values are: 0.5, 1, 4/N (N: total number of observations)

```{r cook_dist}
data_fit<-asd_down_bmi_median %>%
  filter(!is.na(bmi_med)) %>%
  dplyr::select(PATIENT_NUM,bmi_med,DIAGNOSIS) %>%
  mutate(ind=1) %>% spread(DIAGNOSIS,ind,fill=0)

fit1<-glm(bmi_med ~ both+down, data=data_fit)
fit2<-glm(log(bmi_med) ~ both+down, data=data_fit)

ggplot(dat_cook<-data_fit %>% mutate(cooks=cooks.distance(fit1),trans_type="0.original") %>%
         bind_rows(data_fit %>% mutate(cooks=cooks.distance(fit2),trans_type="1.log")),
       aes(x=bmi_med,y=cooks))+
  geom_point(aes(color=trans_type))+
  geom_hline(yintercept = 0.03,linetype=2)
```

* More advanced methods can be found in the chapter 4 in the textbook and can be covered later in class if anyone is intersted.
