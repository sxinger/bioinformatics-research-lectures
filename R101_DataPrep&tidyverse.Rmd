...
*Working example: I want to examine if there is a significant difference of BMI among the three groups of adult patients: just down, just autism (autism spectrum disorder), and both*
...
data_fit<-asd_down_bmi_median %>%
  ...
  dplyr::select(PATIENT_NUM,bmi_median,AGE_AT_FIRSTDX,DIAGNOSIS) %>%
  mutate(ind=1) %>% spread(DIAGNOSIS,ind,fill=0)

fit<-glm(bmi_median ~ both+down+AGE_AT_FIRSTDX, data=data_fit,family="Gamma")
cooks_d<-cooks.distance(fit)

ggplot(dat_cook<-data_fit %>% mutate(cooks=cooks_d,pat_idx=rank(PATIENT_NUM)),
       aes(x=pat_idx,y=cooks))+
  geom_line(aes())+
  geom_hline(yintercept = 0.05,linetype=2,color="blue")+
  labs(x="BMI",y="Cook's D")

ggplot(dat_cook<-data_fit %>% mutate(cooks=cooks.distance(fit),pat_idx=rank(PATIENT_NUM)),
       aes(x=bmi_median,y=cooks))+
  geom_point(aes())+
  geom_hline(yintercept = 0.05,linetype=2,color="blue")+
  labs(x="BMI",y="Cook's D")
...
Let's use the `VIM::aggr` function to explore the missing pattern
...
```{r impute_simple}
##=====================================================
# "replace_na" function for for quick imputation using
# a constant value
##=====================================================
...
asd_down_bmi_median_imp1<-asd_down_bmi_median %>% select(-bmi_median) %>%
...
```{r impute_mice}
bmi_mice<-mice(asd_down_bmi_median[,c("AGE_AT_FIRSTDX","bmi_median")], m=1, maxit = 50, method = 'pmm', seed = 500, printFlag=F)
bmi_mice_imputed<-bmi_mice$imp$bmi_median
asd_down_bmi_median_imp2<-asd_down_bmi_median %>% select(-bmi_median) %>%
  mutate(bmi_median=complete(bmi_mice,1)$bmi_median)
