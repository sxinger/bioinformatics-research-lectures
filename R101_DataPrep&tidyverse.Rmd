...
#### Data Cleaning - Outlier (Chapter 14)
...
Let's use the `aggr` and `plot` function to explore the missing pattern

```{r plot_missing}
# calculate missing proportion
sum(is.na(asd_down_bmi_median$bmi_med))/nrow(asd_down_bmi_median)

# visualize univariate missing pattern
a <- aggr(asd_down_bmi_median, plot = FALSE)
plot(a, numbers = TRUE, prop = FALSE)

# visualize bivariate missing pattern
x <- asd_down_bmi_median[, c("asd_down_bmi_median", "age")]
marginplot(x)
```
