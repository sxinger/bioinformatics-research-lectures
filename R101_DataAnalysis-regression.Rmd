...
#load packages
pacman::p_load(tidyverse,pROC,ResourceSelection,Hmisc,survival,survminer)
...
> To estimate if age_at_bmi is associated with bmi among adult IDD patients
...
```{r bmi_age_lm}
fit1<-glm(log(bmi_median) ~ log(AGE_AT_FIRSTDX), data=idd_bmi)
...
* According to p-value (0.467) of coefficient for age, we conclude that there is no statistical significant association between age at IDD onset and bmi among IDD patients (Autism or Downs) based on the provided data.
...
> To estimate if bmi is a risk factor for developing obstructive sleep apnea (OSA) among IDD patients
...
Let's take a closer look at the model and interpret the results. As you can see, the `Estimate` of `bmi_overwt_ind` (or b1, or coefficient) is 1.2524. By taking the exponential of the coefficient, we get exp(1.2524) = 3.4989 (corresponding to the Odds ratio we calculated above, 3.4989). 
...
* Predictions: in logistic regresion, the predictions are the predictted probability of your target event (outcome). For example, in our model of sleep apnea and bmi, the prediction is the "predictted probability of an autism patient developing sleep apnea":
